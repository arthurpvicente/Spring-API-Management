...
class OutgoingControllerTest {
    ...
    @Test
    void findById_withValidId_returnsOutgoing() throws Exception {
        mockMvc.perform(get("/outgoings/1")
                        .header("Authorization", "Bearer " + token))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.id").value(1))
                .andExpect(jsonPath("$.title").value("Clothes"));
    }

    @Test
    void findById_withInvalidId_returnsNotFound() throws Exception {
        mockMvc.perform(get("/outgoings/999")
                        .header("Authorization", "Bearer " + token))
